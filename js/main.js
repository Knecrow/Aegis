...
if (document.readyState === 'loading') {
  ...
  safeCreateIcons();
    ...
    const state = {
      persona: 'jarvis', // 'jarvis' or 'ultron'
    ...
    /* Authentic MCU Personality Welcome Quotes */
    ...
    const personaSubtitle = document.getElementById('persona-subtitle');
    ...
    /* ==========================================================================
       CANVAS BACKGROUND ENGINE (Hexagonal Grid + Reactive Floating Particles)
       ========================================================================== */
    ...
    /* Smooth Color Transition State (JARVIS: 0, ULTRON: 1) */
    let themeColorProgress = 0; // 0 = JARVIS Cyan, 1 = ULTRON Crimson
    ...
    /* Nanotech Swarm Bot Physics Unit (Avatar Orbital & Talking Reactive Engine) */
    class SwarmBot {
      constructor(index) {
        this.index = index;
        this.reset();
      ...
      reset() {
        ...
        this.baseOrbitRadius = Math.random() * 150 + 70; // Orbit tight around Avatar
      ...
      update(targetX, targetY, now) {
        // AI Talking Reactivity Check
        let isTalking = state.isSpeaking || state.isTypewriting || state.isThinking;
        let talkPulse = isTalking ? (Math.sin(now * 0.015 + this.index * 0.25) * 0.5 + 0.5) : 0;

        // Modulate orbit radius with speech rhythm
        let talkOffset = talkPulse * 45;
        this.targetOrbitRadius = this.baseOrbitRadius + talkOffset;
        this.orbitRadius += (this.targetOrbitRadius - this.orbitRadius) * 0.1;

        // Accelerate orbit speed when talking and apply custom swarm speed
        let currentOrbitSpeed = isTalking ? this.orbitSpeed * (2.2 + talkPulse * 1.5) : this.orbitSpeed;
        this.orbitAngle += currentOrbitSpeed * (state.particlesSpeed || 1.0);
        
        let desiredX = targetX + Math.cos(this.orbitAngle) * this.orbitRadius;
        let desiredY = targetY + Math.sin(this.orbitAngle) * this.orbitRadius;

        // Steering force towards avatar orbit path
        ...
        // Apply physics forces & velocity damping
        this.ax = steerX;
        this.ay = steerY;

        this.vx = (this.vx + this.ax) * 0.96;
        this.vy = (this.vy + this.ay) * 0.96;

        let speed = Math.hypot(this.vx, this.vy);
        let maxSpeed = isTalking ? 3.0 : 1.4;
        if (speed > maxSpeed) {
        ...
        // Modulate glow and alpha when talking
      ...
      draw() {
        ...
        // Smoothly interpolate RGB between JARVIS Cyan (0, 210, 255) and ULTRON Crimson (255, 0, 60)
        const rgb = lerpColorRGB(0, 210, 255, 255, 0, 60, themeColorProgress);
        
        if (themeColorProgress > 0.5) {
          // Morph into Ultron Crimson Diamond
          ctx.rect(this.x - this.radius, this.y - this.radius, this.radius * 2, this.radius * 2);
        } else {
          // Morph into Jarvis Electric Blue Glowing Sphere
    ...
    /* Reactive Shockwave Burst on Message Transmission */
    function burstParticles() {
      shockwaveActive = true;
      shockwaveRadius = 10;
      let inputRect = document.querySelector('.chat-input-area').getBoundingClientRect();
      shockwaveOrigin = { 
      ...
      swarmBots.forEach(bot => {
        let angle = Math.atan2(bot.y - shockwaveOrigin.y, bot.x - shockwaveOrigin.x);
        let force = Math.random() * 6 + 4;
        bot.vx += Math.cos(angle) * force;
        bot.vy += Math.sin(angle) * force;
      });
    }

    /* Reactive Ripple on Typing */
    function triggerTypingRipple() {
      typingRippleActive = true;
      typingRippleRadius = 15;
      ...
      swarmBots.forEach(bot => {
        let dist = Math.hypot(bot.x - typingRippleOrigin.x, bot.y - typingRippleOrigin.y);
        if (dist < 250) {
    ...
    function drawSwarmMesh() {
      ...
      // Smoothly interpolate mesh stroke color
      const rgb = lerpColorRGB(0, 210, 255, 255, 0, 60, themeColorProgress);
      ...
      for (let i = 0; i < swarmBots.length; i++) {
        for (let j = i + 1; j < swarmBots.length; j++) {
          ...
          if (dist < maxConnectDist) {
            let alpha = (1 - dist / maxConnectDist) * (isTalking ? 0.55 : 0.35);
    ...
    function drawShockwave() {
      ...
      ctx.strokeStyle = state.persona === 'jarvis' 
        ? `rgba(0, 210, 255, ${Math.max(0, 1 - shockwaveRadius / 500)})`
        : `rgba(255, 0, 60, ${Math.max(0, 1 - shockwaveRadius / 500)})`;
      ctx.lineWidth = 3;
    ...
    function drawHexGrid() {
      ...
      for (let y = 0; y < canvas.height + hexHeight; y += hexHeight * 0.75) {
        for (let x = 0; x < canvas.width + hexWidth; x += hexWidth) {
          let cx = x + ((Math.floor(y / (hexHeight * 0.75)) % 2) * (hexWidth / 2));
          let cy = y;

          ctx.beginPath();
          for (let i = 0; i < 6; i++) {
            let angle = (Math.PI / 3) * i;
            let hx = cx + hexRadius * Math.cos(angle);
            let hy = cy + hexRadius * Math.sin(angle);
    ...
    function renderCanvas(now) {
      ...
      if (now - lastTime >= 1000) {
        if (readoutFps) readoutFps.textContent = fpsCounter;
        fpsCounter = 0;
        lastTime = now;
      }

      // Smoothly interpolate swarm color progress towards target (0 = JARVIS Cyan, 1 = ULTRON Crimson) over ~2.0s
      themeColorProgress += (targetThemeColorProgress - themeColorProgress) * 0.015;

      ctx.clearRect(0, 0, canvas.width, canvas.height);
      drawHexGrid();

      // Dynamic Attractor Target (Avatar Widget position or center screen)
      let avatarRect = avatarWidget.getBoundingClientRect();
      let targetX = avatarRect.left + avatarRect.width / 2 || canvas.width / 2;
      let targetY = avatarRect.top + avatarRect.height / 2 || canvas.height / 2;
      ...
      // Render glowing energy mesh links between nanobots
      drawSwarmMesh();
    ...
    function setPersona(persona) {
      ...
      if (persona === 'ultron') {
        body.classList.remove('theme-jarvis');
        body.classList.add('theme-ultron');
        ...
        body.classList.remove('theme-ultron');
        body.classList.add('theme-jarvis');
    ...
    /* ==========================================================================
       AI CHAT & GEMINI 2.0 FLASH REST API INTEGRATION
       ========================================================================== */
    async function handleSendMessage() {
      ...
      try {
        ...
        soundFx.error();
        triggerScreenShake();
        appendAiMessage(`[TACTICAL ERROR]: System request failed. ${err.message || 'Check API Key & Connection.'}`);
      }
    }
    ...
    async function callGeminiApi(userPrompt) {
      // Real verified Google AI Studio API models
      ...
      for (const model of fallbackModels) {
        try {
          ...
          if (!res.ok) {
            ...
            // If API key is explicitly invalid, stop loop and inform user
            if (errMsg.toLowerCase().includes('api key') || res.status === 400 || res.status === 403) {
      ...
      // Seamless Failover: If online API quota is reached (429), use Emergency Local Tactical Core!
    ...
    function appendAiMessage(text) {
      ...
      // Typewriter Effect & Optional Text-To-Speech
    ...
    function triggerResponseFlash() {
      ...
      setTimeout(() => {
        avatarWidget.classList.remove('response-glow');
      }, 700);
    }
    ...
    /* ==========================================================================
       VOICE RECOGNITION (Speech-to-Text) & WEB AUDIO ANALYSER VISUALIZER
       ========================================================================== */
    ...
    // Real-time Mic Amplitude Web Audio Visualizer
    async function startMicAudioVisualizer() {
      try {
        ...
        function updateRings() {
          ...
          // Scale concentric rings
          let scale1 = 1 + (avg / 128) * 0.4;
          let scale2 = 1 + (avg / 128) * 0.8;
          let scale3 = 1 + (avg / 128) * 1.2;
          ...
          ring3.style.transform = `scale(${scale3})`;
          ring3.style.opacity = avg > 40 ? '0.6' : '0';

          // Pulse Nanotech Swarm orbit radius with real-time mic volume!
          swarmBots.forEach(bot => {
        ...
        console.warn('Audio Visualizer Permission Denied', e);
      }
    }
    ...
    function speakResponse(text) {
      ...
      utterance.onend = () => {
        ...
        avatarWidget.classList.remove('speaking');
      };
      utterance.onerror = () => {
        state.isSpeaking = false;
        avatarWidget.classList.remove('speaking');
    ...
    closeModalBtn.addEventListener('click', closeModalBtn ? closeModalModal : closeSettingsModal);
    ...
    // Chat Input Live Typing Swarm Ripple Trigger
    chatInput.addEventListener('input', () => {
      triggerTypingRipple();
    });

    // Chat Input Keyboard Trigger (Enter Key)
